...
instance Applicative Count where
  pure x = Count (1, x)
  Count (n, f) <*> Count (m, x) = Count (n + m, f x)
...
say Quiet = unwords . fmap (fmap toLower)
say Loud = unlines . fmap (fmap toUpper)
...
orate speech = say <$> ask <*> pure speech
...
wc = (,,) <$> asks length <*> asks (length . words) <*> asks (length . lines)
...
instance Functor (Clog s) where
  fmap f (Clog g) = Clog (second f . g)

-- fmap f (Clog g) = Clog \s ->
--   let (s', x) = g s
--    in (s', f x)
...
instance Applicative (Clog s) where
  pure x = Clog (,x)
  Clog ff <*> Clog g = Clog (uncurry (<&>) . first g . ff)

-- Clog ff <*> Clog g = Clog \s ->
--   let (s', f) = ff s
--    in f <$> g s'
